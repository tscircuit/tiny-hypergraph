...
import { DEFAULT_MIN_VIA_PAD_DIAMETER } from "./compute-region-cost"
...
export interface TinyHyperGraphSectionSolver2Options
  extends TinyHyperGraphSolver2Options {
  MAX_RIPS?: number
...
const applyTinyHyperGraphSectionSolver2Options = (
  solver: TinyHyperGraphSectionSearchSolver2 | TinyHyperGraphSectionSolver2,
  options?: TinyHyperGraphSectionSolver2Options,
) => {
  applyTinyHyperGraphSolver2Options(solver, options)
...
const getTinyHyperGraphSectionSolver2Options = (
  solver: TinyHyperGraphSectionSolver2,
): TinyHyperGraphSectionSolver2Options => ({
  ...getTinyHyperGraphSolver2Options(solver),
...
const createSolvedSolverFromRegionSegments = (
  ...
  const solver = new TinyHyperGraphSolver2(topology, problem, options)
...
class TinyHyperGraphSectionSearchSolver2 extends TinyHyperGraphSolver2 {
  ...
  constructor(
    ...
    applyTinyHyperGraphSectionSolver2Options(this, options)
  ...
  override tryFinalAcceptance() {
    ...
    this.failed = true
    this.error =
      "Section search timed out before completing any active route solution"
    this.stats = {
      ...this.stats,
      rejectedIncompleteSectionStateOnTimeout: true,
    }
  }
...
export class TinyHyperGraphSectionSolver2 extends BaseSolver {
  baselineSolver: TinyHyperGraphSolver2
  ...
  optimizedSolver?: TinyHyperGraphSolver2
  sectionSolver?: TinyHyperGraphSectionSearchSolver2
  activeRouteIds: RouteId[] = []
  ...
  override _setup() {
    ...
    this.sectionSolver = new TinyHyperGraphSectionSearchSolver2(
      this.topology,
      sectionProblem,
      routePlans,
      activeRouteIds,
      ...
      getTinyHyperGraphSectionSolver2Options(this),
    )
  ...
  getSolvedSolver(): TinyHyperGraphSolver2 {
    if (!this.solved || this.failed || !this.optimizedSolver) {
      throw new Error(
        "TinyHyperGraphSectionSolver2 does not have a solved output yet",
      )
    }
