...
test("final routing moves blockers only within the shared rerip budget", () => {
  for (const parentRipCount of [0, 8]) {
    const solver = new SelectiveReripTinyHyperGraphSolver(
      {
        ...
        portAngleForRegion1: new Int32Array([
          18000, 0, 27000, 27000, 0, 9000, 0, 18000, 0, 9000,
        ]),
        portAngleForRegion2: new Int32Array([
          0, 0, 0, 0, 18000, 18000, 9000, 0, 18000, 27000,
        ]),
    ...
    solver.state.ripCount = parentRipCount
    ...
    expect(solver.stats.finalRoutingStrategy).toBe(
      parentRipCount < 8 ? "congestion-aware" : "greedy",
    )
    if (parentRipCount < 8) {
