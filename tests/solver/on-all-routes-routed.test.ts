...
test("completed routing rerips when a region exceeds the current threshold", () => {
  ...
  solver.state.regionCongestionHistory = [Float64Array.from([0, 0.2])]
...
test("initial congestion maps seed the first solve and congestionWeight affects path cost", () => {
  const solver = createTestSolver({
    congestionAvgWindowSize: 3,
    congestionWeight: 2,
    congestionFalloff: 1,
    initialRegionCongestionCost: new Float64Array([0.9, 0.3]),
  })

  expect(Array.from(solver.state.regionCongestionCost)).toEqual([0.9, 0.3])
