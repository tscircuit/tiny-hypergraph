...
test("pipeline returns full-connection reroutes after section optimization", () => {
  ...
  const reroute = solver.getSolver<FullConnectionRerouteSolver>(
    "rerouteFullConnections",
  )!
  expect(reroute.solved).toBe(true)
  expect(reroute.stats.finalMaxRegionCost).toBeLessThanOrEqual(
    reroute.stats.initialMaxRegionCost,
  )
