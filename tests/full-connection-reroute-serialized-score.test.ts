...
test("rerouting never regresses the serialized output's whole-graph region score", () => {
  for (const graph of [
    ...
    const reroute = new TinyHyperGraphSectionPipelineSolver({
      serializedHyperGraph: graph,
    })
    reroute.solve()
    expect(reroute.solved).toBe(true)
    const summaries = [
      reroute.getStageOutput<
        import("@tscircuit/hypergraph").SerializedHyperGraph
      >("optimizeSection")!,
      reroute.getOutput()!,
    ].map((output) => {
      expect(output.solvedRoutes!.length).toBe(graph.connections!.length)
      const loaded = loadSerializedHyperGraph(output)
      return new TinyHyperGraphSectionSolver(
        loaded.topology,
        loaded.problem,
        loaded.solution,
      ).baselineSummary
    })
    expect(summaries[1]!.maxRegionCost).toBeLessThanOrEqual(
      summaries[0]!.maxRegionCost,
    )
    if (summaries[1]!.maxRegionCost === summaries[0]!.maxRegionCost) {
      expect(summaries[1]!.totalRegionCost).toBeLessThanOrEqual(
        summaries[0]!.totalRegionCost,
      )
    }
  }
})
