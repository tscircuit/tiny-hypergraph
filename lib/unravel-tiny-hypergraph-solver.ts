...
type IntersectionKind = "same-layer" | "transition-pair" | undefined

const classifyLegacyIntersectionLayerMasks = (
  firstLayerMask: number,
  secondLayerMask: number,
): IntersectionKind =>
  (firstLayerMask & secondLayerMask) !== 0 ? "same-layer" : "transition-pair"

const classifyDownstreamRiskLayerMasks = (
  firstLayerMask: number,
  secondLayerMask: number,
): IntersectionKind => {
...
const classifySegmentRiskLayerMasks = (
  firstLayerMask: number,
  secondLayerMask: number,
): IntersectionKind => {
  const firstChangesLayer =
...
const countLegacyIntersectionsWithValuesInto = (
  existingPairs: RegionIntersectionCache,
  newNet: number,
  ...
  for (let index = 0; index < netIds.length; index++) {
    if (newNet === netIds[index]) continue
    const lesserAngleIsInsideInterval =
      newLesserAngle < lesserAngles[index]! &&
      lesserAngles[index]! < newGreaterAngle
    const greaterAngleIsInsideInterval =
      newLesserAngle < greaterAngles[index]! &&
      greaterAngles[index]! < newGreaterAngle
    if (lesserAngleIsInsideInterval === greaterAngleIsInsideInterval) continue
    if ((newLayerMask & layerMasks[index]!) !== 0) {
...
interface PreparedRoutingRiskCapacity {
...
const computeRoutingRisk = (
  capacity: PreparedRoutingRiskCapacity,
  ...
  if (
    ...
    (sameLayerIntersections > 0 ||
      transitionPairIntersections > 0 ||
      entryExitLayerChanges > 0)
  ) {
    return 1
  }
  const estimatedViaCount =
    sameLayerIntersections * 0.82 +
    entryExitLayerChanges * 0.41 +
    transitionPairIntersections * 0.2
  ...
  if (capacity.totalCapacity <= 0) return estimatedUsedCapacity > 0 ? 1 : 0
...
export interface UnravelTinyHyperGraphSolverOptions
  ...
  /** Optional scalarizations for the route-replacement search portfolio. */
...
class SingleRouteReplacementSolver extends TinyHyperGraphSolver {
  ...
  constructor(
    ...
    super(
      ...
      {
        ...getTinyHyperGraphSolverOptions(inputSolver),
        // Search the existing core region-cost objective. Physical-risk and
        // endpoint-clearance invariants are checked on completed candidates.
        STATIC_REACHABILITY_PRECHECK: false,
  ...
  private getActiveReplacementRegionCostMemo(
    ...
    const ownerId = this.getCurrentLegacyOwnerId()
  ...
  private getCurrentLegacyOwnerId(): number {
    if (this.state.currentRouteNetId === undefined) {
      throw new Error("Replacement scoring requires the active route net id")
    }
  ...
  private countOwnerChordIntersections(
    ...
    for (const index of ownerSegmentIndexes) {
      ...
      const intersectionKind = classifyLegacyIntersectionLayerMasks(
        newLayerMask,
  ...
  protected override computeRegionCostAfterAddingSegment(
    ...
    if (packedCounts === 0) {
      ...
      crossingLayerIntersections = this.replacementIntersectionCountsScratch[1]!
      packedCounts =
      ...
      crossingLayerIntersections = counts % memo.scorer.intersectionCountStride
    }
...
export class UnravelTinyHyperGraphSolver extends TinyHyperGraphSolver {
  ...
  // Search both additive marginal cost and an equal-weight congestion
  // scalarization. The first finds the cheapest replacement; the second
  // targets the minimax bottleneck that an additive path cost can otherwise
  // hide. Both are universal objective-derived searches, and every completed
  // candidate is compared by the exact whole-graph objective below.
  ...
  private readonly routingRiskCapacityByRegion: PreparedRoutingRiskCapacity[]
  ...
  private computePhysicalRoutingRisksForRegion(
    ...
    for (let leftIndex = 0; leftIndex < lesserAngles.length; leftIndex++) {
      for (
        ...
        const intersectionKind = classifyDownstreamRiskLayerMasks(
    ...
    for (let leftIndex = 0; leftIndex < segments.length; leftIndex++) {
      ...
      for (
        ...
        if (!intersects) continue
        const intersectionKind = classifySegmentRiskLayerMasks(
          left.layerMask,
          right.layerMask,
        )
        if (intersectionKind === "same-layer") {
  ...
  private findBestRerouteMutation(
    ...
    const scorePreparedCandidate = (
      ...
      candidateSolver.rescoreForOptimizer()
    ...
    for (const regionId of peakRegionIds) {
      ...
      for (let leftIndex = 0; leftIndex < chords.length; leftIndex++) {
        ...
        for (
          ...
          if (
            !intersects ||
            !classifyLegacyIntersectionLayerMasks(
              left.layerMask,
              right.layerMask,
            )
          ) {
  ...
  private getIntersectionOwnerId(routeId: RouteId): number {
    return this.problem.routeNet[routeId]!
  }
